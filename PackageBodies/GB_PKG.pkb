...
-- GB_PKG  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY OLIVER.GB_PKG AS
    ...
    BEGIN
        IF
            NVL(BTYPE,'X') = 'V'
        THEN---Added by Ramana on 200418
            SELECT
                LTRIM(RTRIM(MAX(A.THW_WAIVER_COVERAGE) ) )
            INTO
                VB
            ...
            WHERE
                ...
                AND   NVL(A.THW_START_DATE,DT) = (
                    ...
                    WHERE
                        ...
                        AND   B.THW_PLAN = PL
                        AND   B.THW_MEM_ID = ID1
                        AND   B.THW_BENEFIT = BG
                        AND   NVL(B.THW_START_DATE,DT) <= DT
                );---Up to here 

            IF--Added by Ramana on 200418 
                VB IS NULL OR VB='0'
            THEN
                SELECT
                    *
 ...
 -- SELECT DECODE(DECODE(NVL(VREC.BVB_VOLUME,0),0,NVL(VREC.BVB_NEM,0),NVL(VREC.BVB_VOLUME,0)),0,DECODE(NVL(VREC.BVB_VOLUME,0),0,NVL(VREC.BVB_MAX,0),NVL(VREC.BVB_VOLUME,0)),0) INTO VOL1 FROM DUAL;
   ...
   --VB:=to_char(GET_DEP_STATUS(CID,PL,ID1,BG,DT));
        ...
        ELSIF NVL(BTYPE,'X') = 'D' THEN
            ...
            IF
                DS IS NULL
            THEN
                DS := GET_DEP_STATUS(CID,PL,ID1,BG,DT);--Added by Ramana on 190418
                --DS := GET_DEP_STATUS(CID,PL,ID1,BENEFIT,DT); Commented by Ramana on 190418
            END IF;
