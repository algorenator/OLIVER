--
-- VW_ANNUAL_PORTAL  (View) 
--
CREATE OR REPLACE FORCE VIEW OLIVER.VW_ANNUAL_PORTAL
...
SELECT
        ...
        "ANN_COMP",
        "ANN_EE_CONTS",
        DECODE(NVL(ANN_STATUS,'C'),'TNV',0,ANN_ER_CONTS),
        "ANN_EARNINGS",
        "ANN_HRS",
        "ANN_CRED_SERV",
        "ANN_PA",
    ...
    || TO_CHAR(ann_year),'DD-MON-RRRR'),'ER',ann_account),DECODE(NVL(ANN_STATUS,'C'),'TNV',0,ER_INT) ) er_int,
    ...
    "ANN_ACCOUNT"
    FROM
        tbl_annual WHERE NVL(ANN_STATUS,'C') not in ('W');
